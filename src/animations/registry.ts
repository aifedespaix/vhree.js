...
export const registerAnimation = <Opts = unknown>(name: string, factory: AnimationFactory<Opts>): void => {
...
const resolveFactory = (name: string | undefined): AnimationFactory<unknown> | undefined => {
  if (!name) return undefined
  return registry.get(name)
}

export const resolveAnimation = (spec: AnimationSpec | null | undefined): Animation | null => {
  if (!spec) return null
